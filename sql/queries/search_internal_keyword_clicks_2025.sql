...
-- CREATE OR REPLACE TABLE `rome-prod.temp.search_internal_keyword_clicks_2025` AS

WITH filtered_base AS (
  SELECT
    get_business_channel(account_id, device_type) AS business_channel,
    DATE(datetime, "Asia/Jakarta") AS date,
    ...
    extension_data,
    search_result_count
  FROM `geneva-prod.bwa.bwa_data_search_click`
  WHERE 1=1
...
base AS (
  ...
  FROM filtered_base
  WHERE 1=1
    AND session_id in (SELECT session_id FROM valid_sessions)
    AND session_id NOT IN (SELECT session_id FROM excluded_sessions)
    AND LOWER(search_internal_keyword) NOT IN (SELECT keyword FROM stopwords)
),
...
extension_agg AS (
  SELECT
    ...
    MAX(IF(ed.key = 'discount', ed.val, NULL)) AS discount,
...
SELECT
  b.* EXCEPT (extension_data),
  e.* except(search_id,search_internal_keyword,session_id)
FROM base b
LEFT JOIN extension_agg e
ON  b.search_id = e.search_id
AND b.session_id = e.session_id
AND b.search_internal_keyword = e.search_internal_keyword;